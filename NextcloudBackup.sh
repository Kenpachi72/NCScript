...
if [ "${databaseSystem,,}" = "mysql" ] || [ "${databaseSystem,,}" = "mariadb" ]; then
	...
	if ! [ -x "$(command -v mysqldump)" ]; then
		...
		mysqldump --single-transaction -h "${mysqlhost}" -u "${dbUser}" -p"${dbPassword}" "${nextcloudDatabase}" > "${backupDir}/${fileNameBackupDb}"
	fi
	...
	if ! [ -x "$(command -v pg_dump)" ]; then
		...
		PGPASSWORD="${dbPassword}" pg_dump "${nextcloudDatabase}" -h "${mysqlhost}" -U "${dbUser}" -f "${backupDir}/${fileNameBackupDb}"
	fi
